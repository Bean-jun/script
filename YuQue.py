...
try:
    from yuque_localsetting import *
except ImportError:
    pass
    ...
    def _request(self, method, path, **kwargs):
        try:
            ...
            if isinstance(method, str):
                response = requests.request(method.upper(), self.BASE_URL + path, data=json.dumps(kwargs), headers=self.headers)
            else:
...
if __name__ == "__main__":
    client = YuQueConnect(Token, HEADERS)
    # 创建知识库测试
    detail = {
        "name": "test_",
        "slug": "test_",
    ...
    res = client.create_docs('shujuku', **blog_detail)
    # res = client.update_docs('zhangsan', 45414431, **update_blog_detail)
    print(res)
    if res['data']:
        print('ok')
